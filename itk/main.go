...
func (e *V10AgentExecutor) Execute(ctx context.Context, execCtx *a2asrv.ExecutorContext) iter.Seq2[a2a.Event, error] {
	return func(yield func(a2a.Event, error) bool) {
		log.Info(ctx, "Executing task", "taskId", execCtx.TaskID)
		...
		if err != nil {
			log.Error(ctx, "Failed to extract instruction", err)
...
func (e *V10AgentExecutor) Cancel(ctx context.Context, execCtx *a2asrv.ExecutorContext) iter.Seq2[a2a.Event, error] {
	return func(yield func(a2a.Event, error) bool) {
		log.Info(ctx, "Cancel requested", "taskId", execCtx.TaskID)
...
func main() {
	if err := run(); err != nil {
		log.Error(context.Background(), "Server session ended with error", err)
		os.Exit(1)
	}
}
